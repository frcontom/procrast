      ...
      <div className="flex gap-3">
        {[
          ...
          <button key={t.id} onClick={() => setTheme(t.id as 'dark' | 'light')}
            className={`flex items-center gap-2 px-4 py-3 rounded-lg text-sm transition-colors flex-1 ${theme === t.id ? 'bg-[var(--accent)] text-white' : 'bg-secondary text-text-secondary hover:text-white'}`}>
            <span>{t.icon}</span>
            <span>{t.label}</span>
          </button>
        ))}
