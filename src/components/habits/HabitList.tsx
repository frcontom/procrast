...
export function HabitList({ habits, logs, daysInMonth, onDelete, onEdit, onReorder, onTogglePrimary, onToggleStarred }: Props) {
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => onToggleStarred?.(habit.id, !habit.is_starred)} className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs border transition-all ${habit.is_starred ? 'bg-[#FFD700]/20 border-[#FFD700]/40 text-[#FFD700]' : 'bg-white/5 border-white/[0.06] text-white/20 hover:text-[#FFD700] hover:border-[#FFD700]/30'}`} title={habit.is_starred ? 'Quitar super importante' : 'Super importante'}>⭐</button>
