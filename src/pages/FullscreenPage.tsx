  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = color.replace(')', `,${Math.max(0, p.alpha)})`).replace('rgb', 'rgba')
        ctx.fill()
      })
      frame++
...
export function FullscreenPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('sessions').select('id', { count: 'exact' }).eq('user_id', user.id).eq('state', 'completed').gte('started_at', today).then(({ count }: any) => setTodaySessions(count || 0))
  ...
  useEffect(() => {
    if (justFinished) {
      ...
      scoreTimerRef.current = window.setTimeout(() => navigate('/focus'), 3000)
    }
  ...
  const displayPct = Math.max(0.5, pct)
  ...
  return (
    <div id="fullscreenOverlay" className="fixed inset-0 z-[2000] flex items-center justify-end overflow-hidden bg-black select-none">
      ...
      {showParticles && <ParticleBurst color={ringColor} />}
      ...
      <div id="fullscreenTimerRingContainer" className={`absolute z-[2] ${soloMode ? 'inset-0 flex items-center justify-center' : 'relative'}`}
        style={soloMode ? { position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' } : {}}>
        <div className={soloMode ? '' : ''} style={{ width: soloMode ? 'min(80vw, 80vh)' : '360px', height: soloMode ? 'min(80vw, 80vh)' : '360px' }}>
          <div className="relative w-full h-full">
            ...
            <div id="fullscreenRingFill" className="absolute inset-0 rounded-full transition-all duration-300"
              style={{
                background: `conic-gradient(${ringColor} ${displayPct}%, rgba(255,255,255,0.04) ${displayPct}%)`,
                WebkitMask: 'radial-gradient(circle at center, transparent 65%, black 65%)',
                mask: 'radial-gradient(circle at center, transparent 65%, black 65%)',
      ...
      {!soloMode && (
        <>
          {showBreathing && (
            <div id="fullscreenBreathe" className="relative z-[2] flex flex-col items-center gap-5 animate-[breatheFadeIn_0.5s_ease]">
          ...
          <div id="fullscreenProgress" className="relative z-[2] w-full h-[3px] bg-white/10 rounded overflow-hidden max-w-[650px] w-1/2 pr-10">
            <div className="h-full rounded transition-all duration-500 ease" style={{ width: `${Math.min(100, pct)}%`, backgroundColor: ringColor }} />
          </div>

          <div id="fullscreenInfo" className="relative z-[2] flex gap-4 text-lg text-white/60">
          ...
          {showMiniStats && (
            <div id="fullscreenMiniStats" className="relative z-[2] flex items-center gap-0 text-lg text-white/70 animate-[breatheFadeIn_0.3s_ease]">
              ...
              <span className="px-5 border-r border-white/10">📊 <span id="fsToday">{todaySessions}</span> hoy</span>
              <span className="px-5">🎯 {todaySessions}/4 ses</span>
            </div>
          )}

          <div id="fullscreenActions" className="relative z-[2] flex gap-3 w-full justify-center max-w-[650px] w-1/2 pr-10">
